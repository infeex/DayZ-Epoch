	...
	for "_i" from _countpos to 1 step -1 do {
		...
		if ((count _x == 3) AND {(dayz_currentWeaponHolders < dayz_maxMaxWeaponHolders)}) then {	
			...
			if (((!_local) OR {(_local AND {(_existing_loopiles_count == 0)})}) AND {(_lootChance > _rnd)}) then {
				...
				if (((!isNil "_itemTypes") AND {(!isNil "_index")}) AND {((_index >= 0) AND {(_index < count _itemTypes)})}) then {
					_itemType = _itemTypes select _index;
					//[_itemType select 0, _itemType select 1, _iPos, 0.0] call spawn_loot;
					_this set [ count _this, [_itemType select 0, _itemType select 1, _iPos, 0.0] ];
...
dayz_sg_newloot call _spawnLoot;

_itemTypes = [] + getArray (_config >> "itemTypeSmall");
_pos = [] + getArray (_config >> "lootPosSmall");
dayz_sg_newlootSmall call _spawnLoot;
