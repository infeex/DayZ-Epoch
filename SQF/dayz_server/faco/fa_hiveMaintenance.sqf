/*
        Created exclusively for ArmA2:OA - Epoch-DayZMod.
...
fa_coor2str = {
	...
	_res = format["%1 [%2:%3]", _town, floor((_pos select 0)/100), floor((15360-(_pos select 1))/100)];
...
#ifdef VEH_MAINTENANCE_RECYCLE	
						(_myArray select _forEachIndex) set [8, 0.99] ; // damage = 0.99 so this veh will me respawned
